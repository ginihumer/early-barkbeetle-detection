...
COLOR_DICT = np.array([Unlabeled, Tree, Sick, Dead])

THRESHOLD = 70 # all colors which lay within this threshold are of this class; This is because of slight changes within the image, which can happen during augmentation or loading; Take care for this when creating the mask
...
def custom_preprocess_input(img): # use this function as parameter when training without transfer learning
    return img / 255
...
def adjustData(img,mask,flag_multi_class, num_class, true_value = 1): # num_class is obsolete, since in this version of the method it isn't used
    ...
    if(flag_multi_class):
#         img = img / 255 #is implied in "preprocessing function" parameter of Data Generator
...
def trainGenerator(batch_size,train_path,image_folder,mask_folder,aug_dict,image_color_mode = "grayscale",
                    ...
                    flag_multi_class = False,num_class = 2,save_to_dir = None,target_size = (256,256),seed = 1, true_value = 1, preprocessing_function=None):
    '''
    can generate image and mask at the same time
    use the same seed for image_datagen and mask_datagen to ensure the transformation for image and mask is the same
    if you want to visualize the results of generator, set save_to_dir = "your path"
    '''
    image_datagen = ImageDataGenerator(preprocessing_function=preprocessing_function, **aug_dict)
